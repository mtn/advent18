...
class Goblin(object):
    ...
    def attack(self):
        attack((self.y, self.x), self.attack_power, Elf)
...
class Elf(object):
    ...
    def attack(self):
        attack((self.y, self.x), self.attack_power, Goblin)
...
def setup():
    ...
    return grid, movers, grid_width, grid_height
...
elf_attack_level = 3
while True:
    grid, movers, grid_width, grid_height = setup()
    start_elves = len(list(filter(lambda x: isinstance(x, Elf), movers)))

    for m in movers:
        if isinstance(m, Elf):
            m.attack_power = elf_attack_level
    ...
    while num_elves != 0 and num_elves != len(movers):
        ...
        for i, m in enumerate(movers):
            ...
            if num_elves == 0 or num_elves == len(movers):
                ...
                if num_elves == len(movers):
                    print(sum_hp * round_no)
                    exit()
                break
